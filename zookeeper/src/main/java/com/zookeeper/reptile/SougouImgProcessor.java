...
public class SougouImgProcessor {
    ...
    public void pipelineData(){
        // 多线程
        pipeline.procesçsSync(this.urlList, this.word);
    }


    public static void main(String[] args) {
        String url = "https://pic.sogou.com/napi/pc/searchList?mode=1&start=%s&xml_len=%s&query=%s";
        SougouImgProcessor processor = new SougouImgProcessor(url,"鞠婧祎");

        int start = 0, size = 50, limit = 300 ; // 定义爬取开始索引、每次爬取数量、总共爬取数量
